...
//$(function() {
//    $('<div id="minitoc" class="dontprint"></div>').prependTo("#left-panel-wrapper");
//});
...
function generateMiniToc(divId) {
    ...
    $('#' + divId).find('h3').each(function(i) {
        ...
        var secelt = $("<div>" + $(this).html() + "</div>");
        secelt.find('span').remove();
        var sechead = secelt.text().trim();
        $("#minitoc").append("<li><a href='#" + $(this).attr("id") + "'>"
                             + sechead + "</a></li>");
    });
...
$(document).ready(function() {
    ...
    // If no hash, build the minitoc anyway for selected tab
        ...
        var divId = $('#content div[aria-hidden=false]').attr('id');
        generateMiniToc(divId);
    }
...
function togglePanel(e) {
    ...
    var left_panel = $("#left-panel-contents");
    left_panel.toggleClass('active');
    // $("#right-panel-contents").toggleClass('active').toggle(200);

    if (left_panel.hasClass('active')) {
        left_panel.animate({'left': "0px"}, {duration: '200'});
        $("#content").animate({'margin-left': left_panel.outerWidth()+"px"}, {duration: '200'});
        $("#left-panel-button").
            animate({"left": "182px", "opacity": 0.9}, {duration: "200" });
    } else {
        left_panel.animate(
          {'left': "-" + (left_panel.outerWidth() - 2) + "px"},
          {duration: "200"}
        );
        $("#content").animate({'margin-left': "0px"}, {duration: '200'});
        $("#left-panel-button").
            animate({"left": "-23px", "opacity": 0.9}, {duration: "200" });
    }
...
$(function() {
    $('<div id="left-panel-wrapper" class="dontprint">'
      + '<div id="left-panel-contents" style="opacity: 0.9"></div>'
      + '<div id="left-panel-button" class="dontprint"><a href="#">Dashboard</a></div>'
      + '</div>')
        .appendTo('body');

    /* $('<div id="right-panel-wrapper" class="dontprint">'
      + '<div id="right-panel-contents" style="opacity: 0.9"></div>'
      + '</div>')
        .appendTo('body'); */
 ...
 */
$(function() {
    $('<b>Section Contents</b>:<br/><ul id="minitoc"></ul>').appendTo("#left-panel-contents");
    var divId = $('#content div[aria-hidden=false]').attr('id');
    generateMiniToc(divId);
...
$(function() {
    var ul = $('<ul id="listOfTags"></ul>').appendTo('#left-panel-contents');
...
$(function() {
    $('.tag span').click(function(e) {
        ...
        $('#left-panel-wrapper .' + orgTag).parent().parent()
            .toggleClass('matchtag');
        e.preventDefault();
    })
});
...
function orgDefkey(e) {
    ...
    // Was Esc char (27) pressed, or pressed last?
    if (!esc_last_keypress) {
        if (keycode === esc_keycode) esc_last_keypress = true;
        return;
    }
    else {
        esc_last_keypress = false;
        if (keycode === esc_keycode) {
            hsReviewTaskQuit();
            if ($("#left-panel-contents").hasClass('active')) {
                //&& $("#right-panel-contents").hasClass('active')) {
                togglePanel(e);
            }
        }
    }
    ...
    switch (actualkey) {
        ...
        case "+": // expand all
            hsExpandAll();
            break;
        case "f": // go to next task
            hsReviewTaskNext();
            break;
        case "b": // go to previous task
            hsReviewTaskPrev();
            break;
        case "g": // quit reviewing
            hsReviewTaskQuit();
            break;
        case "j": // refresh the page (from the server, rather than the cache)
